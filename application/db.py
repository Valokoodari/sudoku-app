...
def get_sudoku(id: int) -> list:
    sql = text("SELECT name, cells, instructions, display, owner_id " \
          "FROM sudokus " \
          "WHERE id = :id")
...
def get_public_sudokus() -> list:
    sql = text("SELECT sudokus.id, sudokus.name, users.display_name " \
          "FROM sudokus, users " \
...
def get_sudoku_shares(sudoku_id: int) -> list:
    sql = text("SELECT user_id FROM shares WHERE sudoku_id = :sudoku_id")
...
def add_sudoku(user_id: int, name: str, cells: list, instructions: str, display: str) -> int:
    sql = text("INSERT INTO sudokus (owner_id, name, cells, instructions, display) " \
    ...
    result = db.session.execute(text("SELECT MAX(id) FROM sudokus"))
...
def share_sudoku(sudoku_id: int, user_id: int):
    sql = text("INSERT INTO shares (sudoku_id, user_id) " \
...
def set_sudoku_display(sudoku_id: int, display: str):
    sql = text("UPDATE sudokus SET display = :display WHERE id = :id")
...
def get_user(username: str) -> list:
    sql = text("SELECT id, password_hash, display_name FROM users " \
...
def create_user(username: str, password_hash: str, display_name: str) -> list:
    sql = text("INSERT INTO users (username, password_hash, display_name, permission_level) " \
...
def add_comment(user_id: int, sudoku_id: int, rating: int, content: str):
    sql = text("INSERT INTO comments (user_id, sudoku_id, rating, content) " \
...
def get_comments_by_sudoku(sudoku_id: int) -> list:
    sql = text("SELECT users.display_name, comments.rating, comments.content " \
          "FROM comments, users " \
